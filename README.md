...
The paper replaces the step-function thresholds of the Linear Threshold Model
(LTM) with the saddle-node bifurcations of Nonlinear Opinion Dynamics (NOD),
turning a discrete cascade process into a continuous flow. This repository
contains that model, the 30-node LTM instance used in the paper, and the scripts
that generate the two simulation figures.
...
| `test_ltm_nod.py` | Numerical checks of Theorems 1 and 2, and of the Fig. 3 claim, on the shipped instance. Run it directly; no test runner needed. |
| `data/` | The instance `(A, tau)` used for every figure. |
| `figures/` | Generated output. |

Each script keeps its parameters as constants at the top — seed set, `k`, the
`gamma` fractions, input amplitude and timing, and a `LAYOUTS` dict of printed
sizes. Changing those and re-running is the intended way to explore.
